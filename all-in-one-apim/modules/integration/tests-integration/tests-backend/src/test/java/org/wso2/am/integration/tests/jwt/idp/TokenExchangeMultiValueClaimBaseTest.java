...
/**
 * Base test for token exchange custom claim handling with multi-value groups claims.
 */
public abstract class TokenExchangeMultiValueClaimBaseTest extends APIManagerLifecycleBaseTest {
    ...
    protected TokenExchangeMultiValueClaimBaseTest(TestUserMode userMode) {

        this.userMode = userMode;
    }

    protected abstract List<String> getExpectedGroups();
    ...
    public void setupTokenExchangeMultiValueClaimTest() throws Exception {
        ...
        super.init(userMode);
        keyManagerName = "TokenExchangeMultiValueClaimKM-" + getTestNameSuffix();
        tokenEndpointURLString = getKeyManagerURLHttps() + "/oauth2/token";

        HttpResponse applicationResponse = restAPIStore.createApplication(
                "TokenExchangeMultiValueClaimApp-" + getTestNameSuffix(), "JWT Application",
    ...
    @Test(groups = { "wso2.am" }, description = "Validate token exchange groups claim handling for ArrayList claims")
    public void testTokenExchangeGroupsClaimHandlingForArrayListClaims() throws Exception {
        ...
        Assert.assertEquals(exchangedAccessTokenClaims.getStringListClaim("groups"), getExpectedGroups(),
    ...
    private String createKeyManager() throws Exception {
        ...
        claimMappings.add(new ClaimMappingEntryDTO().remoteClaim("groups").localClaim("http://wso2.org/claims/groups"));
        claimMappings.add(new ClaimMappingEntryDTO().remoteClaim("preferred_username")
